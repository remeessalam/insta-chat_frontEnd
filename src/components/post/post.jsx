...
function Post({ e }) {
    ...
    return (
        <div key={e._id} className=" flex flex-col  mx-auto rounded-md border border-slate-300 lg:w-3/4 w-full mb-3 drop-shadow-l">
            ...
            <div className="" >
                <div className='flex overflow-x-auto snap-x snap-mandatory scrollbar-hide bg-black' onClick={() => {
                    setOpen(!open)
            ...
            <div className="flex flex-col justify-start border-t  p-3">
                <div className="flex flex-row ">
                    {liked ?
                        ...
                        <div className="ml-3 flex flex-row cursor-pointer" onClick={() => { like(e._id) }}>
                            <FavoriteBorderRoundedIcon />
                            <h1>like</h1>
                        </div>
                        :
                        <div className="ml-3 flex flex-row cursor-pointer" onClick={() => {
                            like(e._id)
                        }}>
                            <FavoriteIcon />
                            <h1>unlike</h1>
                    ...
                    <div className="ml-8 flex flex-row cursor-pointer" onClick={() => {
                        setOpen(!open)
            ...
            <div className='flex justify-start pl-7'>
                <h1 className='text-sx font-normal'>{e.Likes.length} likes</h1>
            </div>
            <div className='flex flex-row justify-start pl-7'>
                <h1 className='text-sx font-medium'>{e.user.name} </h1>
                <p className='pl-1 pb-4 font-light text-sx'>Vazhak heating up IFFK!</p>
            </div>
