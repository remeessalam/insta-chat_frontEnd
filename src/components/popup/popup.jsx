...
export default function Modal({ open, setOpen, posts }) {
    ...
    return (
        <div className=''>
            {open && (
                <div className="fixed left-[50%] top-[50%]  -translate-y-[50%] -translate-x-[50%] w-full h-full p-1 brightness-">
                    <div className="flex flex-wrap modal-content border w-[350px] lg:w-[1200px] lg:h-[700px] h-[650px] bg-white mx-auto brightness-100 overflow-y-auto scrollbar-hide" >
                        ...
                        <div className='flex items-center sm:w-1/2 w-full h-1/2 sm:h-full bg-black '>
                            <div className="flex items-center" >
                                <div className='flex  h-full overflow-x-auto snap-x snap-mandatory scrollbar-hide bg-black' onClick={() => {
                                    setOpen(!open)
                                    ...
                                    {posts.image.map((obj) => {
                                        return (
                                            ...
                                            <div className=' min-w-full snap-always snap-center'>
                                                ...
                                                <img className='flex h-full max-h-[700px]  mx-auto object-cover aspect-auto' src={obj.url} alt="" />
                                            </div>
                        ...
                        <div className='flex flex-col bg-white  sm:w-1/2 w-full sm:h-full h-[500px] overflow-x-auto '>
                            ...
                            <div className="flex items-center h-20 border-b">
                                <img className="ml-3 rounded-full w-11 h-11 object-cover" src={posts.image[0]?.url} alt="" />
                                <div className=' item-center'>
                                    <h1 className="text-md font-medium ml-3">{posts.user.name}</h1>
                                </div>
                            </div>
                                            ...
                                            return (
                                                <div className='flex flex-row p-5 w-100%'>
                                                    <div className='min-w-28'>
                                                        <p className='font-normal mr-1 w-28 cursor-pointer'>{com.commentBy.name} </p>
                                                    </div>
                                                    <h1 className='text-sx font-medium ml-2 w-full'>{com.comment} </h1>
                                                    <span className='cursor-pointer' onClick={() => deletecomment(posts._id, com._id)}><DeleteOutlineRoundedIcon /></span>
                                                </div>
                                            )
                                        })
                                ...
                                <div className="flex flex-col    p-3">
                                    <div className="flex flex-row ">
                                        {liked ?
                                            ...
                                            <div className="flex flex-row cursor-pointer" onClick={() => { like(posts._id) }}>
                            ...
                            <div>
                                <div className='flex flex-row justify-start pl-7 border-y p-3 w-full'>
                                    ...
                                    <div className='w-1/4 flex justify-end'>
                                        {comment ?
                                            <button className='text-sx font-semibold text-blue-400 ' onClick={() => Comments(posts._id)}>post</button>
