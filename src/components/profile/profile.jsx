                ...
                <div className="flex-col w-full h-full mx-auto ">
                    {IsBigScreen ?
                        <div className="flex flex-row  h-1/2 p-1 sm:p-4">
                            <div className="sm:w-1/4 w-screen pl-16 ">
                                {user?.user?.image ?
                                    <img className="rounded-full  h-36 w-36 border-red-400 object-cover" src={user?.user?.image} alt="" />
                            ...
                            <div className="flex flex-col w-3/4 m-5 pl-9 ">
                                <div className="flex flex-row m-4">
                                    <div >
                                        <h1 className="mr-7 font-normal text-3xl ">{user?.user?.name}</h1>
                                    </div>
                                ...
                                <div className="flex flex-row  m-4" >
                                    ...
                                    <h1 className="mr-7 cursor-pointer" onClick={() => hovercontant(userDetails?.followers, 'Followers')}>followers {userDetails?.followers?.length}</h1>
                                    <h1 className='cursor-pointer' onClick={() => hovercontant(userDetails?.following, 'Following')}>following  {userDetails?.following?.length}</h1>
                                </div>
                                <div className="flex flex-row  m-4">
                                    <p>{userDetails?.bio}
                                    </p>
                        ...
                        <div className="flex flex-col  h-1/2 p-1 lg:p-5">
                            <div className="sm:w-1/4 w-screen m-1">
                                {userDetails.image ?
                                    <img className="rounded-full  h-36 w-36 border-red-400 object-cover" src={userDetails.image} alt="" />
                                    :
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-56 h-56">
                                        <path fillRule="evenodd" d="M18.685 19.097A9.723 9.723 0 0021.75 12c0-5.385-4.365-9.75-9.75-9.75S2.25 6.615 2.25 12a9.723 9.723 0 003.065 7.097A9.716 9.716 0 0012 21.75a9.716 9.716 0 006.685-2.653zm-12.54-1.285A7.486 7.486 0 0112 15a7.486 7.486 0 015.855 2.812A8.224 8.224 0 0112 20.25a8.224 8.224 0 01-5.855-2.438zM15.75 9a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" clipRule="evenodd" />
                                    </svg>
                            ...
                            <div className="flex flex-col w-3/4 m-5">
                                <div className="flex flex-row ">
                                    <div >
                                        <h1 className="mr-7 font-normal md:text-3xl text:xl ">remees salam</h1>
                                    </div>
                    ...
                    <div className="p-5 grid grid-cols-1 md:grid-cols-3 gap-8 ">
                        {
                            post.map((e, i) => {
                                return (
                                    <div key={i} className=" cursor-pointer grid justify-items-start group">
                                        <div className="justify-self-center min-w-80 min-h-80 max-h-80 max-w-80 relative">
                                            <img className='justify-self-center object-cover h-80 w-80 hover:opacity-25'  src={e.image[0]?.url} alt="" />
                                            <div className="flex flex-row  absolute bottom-32 left-24 opacity-0 group-hover:opacity-100 ">
                                                ...
                                                {e.Likes.length ? <h1 className='text-lg font-bold ml-2 mr-2'>{e.Likes.length}</h1> : <h1 className='text-lg font-bold'> 0</h1>}
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
...

                                                {
                                                    ...
                                                    <h1 className='text-lg font-bold ml-2'>{e.comments.length}</h1>
                                                }
                                            </div>
                                        </div>
                                    </div>
